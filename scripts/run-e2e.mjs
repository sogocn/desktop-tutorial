// 在内存 PGlite 上跑一遍 db/local + db/migrations + db/tests/test_cross_account，
// 验证跨账号端到端联调（三个身份依次驱动真实业务函数）。
...
console.log('\n› 运行跨账号 E2E 断言')
notices.length = 0
await run('db/tests/test_cross_account.sql', read('db/tests/test_cross_account.sql'))
